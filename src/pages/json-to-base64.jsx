...
export default function ImagetoBase64() {
  ...
  const valueChanged = (val) => {
    const base64String = Buffer.from(val).toString('base64')
  ...
  return (
    <div>
      <Head>
        <title>JSON to Base64 Converter Online | DevFinds</title>
        <meta
          name="description"
          content="Generate Base64 String from JSON object or array using this tool"
        />
      </Head>
      <Header />
                  ...
                  <div className="sm:grid sm:grid-cols-3 sm:items-start sm:gap-4  sm:pt-5">
                    ...
                    <div className="mt-1 sm:col-span-2 sm:mt-0">
                      {/* <textarea
                        ...
                        name="about"
                        rows={5}
                        onChange={(e) => createBase64String(e.target.value)}
                        className="block w-full max-w-lg rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                        spellCheck={false}
                      /> */}
                      <CodeEditor type={'json'} valueChanged={valueChanged} />
                    </div>
                  </div>
