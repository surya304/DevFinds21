...
export default function Base64Decode() {
  ...
  const decodeResult = (val) => {
    ...
    const text = asciiString
    // const asciiString = buffer.toString('ascii')
    // const asciiArray = []
    // for (let i = 0; i < asciiString.length; i++) {
    //   asciiArray.push(asciiString.charCodeAt(i))
    // }
    // const text = asciiArray.map((code) => String.fromCharCode(code)).join('')
  ...
  return (
    <div>
      <Head>
        <title>Base64 to ASCII Converter Online | DevFinds</title>
        <meta
          name="description"
          content="Generate ASCII characters from Base64 String using this tool"
        />
      </Head>
      <Header />
